...
|    Name     | 归属 | 属性 | 说明                                                         |
| :---------: | :--: | ---- | ------------------------------------------------------------ |
| `JD_COOKIE` | 京东 | 必须 | 京东cookie,多个账号的cookie使用`&`隔开，例：`pt_key=XXX;pt_pin=XXX;&pt_key=XXX;pt_pin=XXX;&pt_key=XXX;pt_pin=XXX;`。|
...
|    `BOOKSHOP_SHARECODES`    |   口袋书店<br>互助码   | 非必须 |            10/            | 填写规则和上面类似，或见下方[互助码的填写规则](#互助码的填写规则) |
...
|       `feedNum`       |        宠汪汪喂食数量        | 非必须 | 控制`jd_joy_new.js`脚本喂食数量,可以填的数字10,20,40,80,其他数字不可. |
|     `JD_JOY_REWARD_NAME`     |  宠汪汪<br>积分兑换多少京豆  | 非必须 | 目前可填值为`20`或者`500`，默认`500`     |
...
|    `HELP_AUTHOR`      |    是否给作者（lxk0301）助力<br>默认是     | 非必须 | 填`false`可关闭此助力 |
|`HELP_HW`|是否为HelloWorld助力<br>默认是|非必须|填`false`可关闭此助力|
|`HELP_POOL`|是否帮助助力池<br>默认是|非必须|填`false`可关闭此助力|
|`CFD_LOOP_DELAY`|财富岛<br>热气球时间间隔<br>默认10000~25000随机|非必须|捡气球间隔时间，单位毫秒|
|`CFD_STOCK`|财富岛<br>监控商品关键词|非必须|添加需要监控的品名，&分隔，例如`CFD_STOCK="必胜客50元美食卡&星巴克50元代金券"`|
|`JXGC_NOTIFY_LEVEL`|京喜工厂<br>推送级别<br>默认2级|非必须|通知级别 1=生产完毕可兑换通知;2=可兑换通知+生产超时通知+兑换超时通知;3=可兑换通知+生产超时通知+兑换超时通知+未选择商品生产通知(前提：已开通京喜工厂活动)|
|`CFD_LOOP_ENABLE`|启用杀掉并重启jd_cfd_loop任务|非必须|默认为`true`，填`false`可关闭|
